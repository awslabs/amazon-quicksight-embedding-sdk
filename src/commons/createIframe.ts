...
const circles = [1, 2, 3]
    .map(
        i => `<circle fill="#ccc" stroke="none" cx="${i * 20 - 14}" cy="50" r="6">
    <animate attributeName="opacity" dur="1s" values="0;1;0" repeatCount="indefinite" begin="${i / 10.0}"/>
</circle>`
    )
    .join();
const loaderSVG = `<svg version="1.1" id="L4" x="0px" y="0px" viewBox="0 0 100 100" enable-background="new 0 0 0 0" style="width:100px; height:100px;">
    ${circles}
</svg>`;
...
const createIframe = (options: CreateIframeOptions): EmbeddingIFrameElement | null => {
    ...
    if (withIframePlaceholder) {
        ...
        if (typeof withIframePlaceholder !== 'boolean') {
            ...
            iframePlaceholder.innerHTML = loaderSVG;
        }
